...
MirobotSimMenu = function(el, mirobot){
  var self = this;
  
  var container = document.querySelector(el);
  container.insertAdjacentHTML('beforeend', '<div id="sim" class="menuItem"></div>');
  this.el = container.querySelector('#sim');
  
  this.localStorage = initLocalStorage();
  this.mirobot = mirobot;
  this.hide = false;
  this.el.classList.remove('hidden');
  this.el.innerHTML = "<span>" + l(':simulate', "Simulate") + '</span> <div class="onoffswitch">\
    <input type="checkbox" name="onoffswitch" class="onoffswitch-checkbox" id="myonoffswitch">\
    <label class="onoffswitch-label" for="myonoffswitch"></label>\
</div>'
  var enable = this.el.querySelector('input');
  ...
  label.innerHTML = l(":100mm-grid", "100mm grid");
  this.sim.appendChild(label);

  var settings = document.createElement('div');
  settings.className = 'settings';
  settings.innerHTML = '<label>' + l(':fast-sim', "Speed up") + '<input type="checkbox" class="fastSim"></label><label>' + l(':hide-sim', "Hide until active") + '<input type="checkbox" class="hideSim"></label>';
  ...
  reset.innerHTML = l(":reset-sim", "Reset");
