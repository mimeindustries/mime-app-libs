MimeConn = function(device, options){
  var self = this;
  ...
  this.connect = function(address){
    if(self.address !== address){
      ...
      self.device.connect('ws://' + address + ':8899/websocket');
      self.updateMenu();
    }
  }
  ...
  this.autoConnect = function(){
    ...
    device.fetchDevices(function(devices){
  ...
  this.updateMenu = function(){
    this.menu.setDevices(device.devices, this.address);
  ...
  this.init = function(){ 
    ...
    this.extractConfig();
    this.menu = new MimeConnMenu('conn')
...
MimeConnMenu = function(el){
  ...
  this.setDevices = function(devices, selected){
    ...
    if(typeof this.devices[selected] === 'undefined'){
      this.devices[selected] = {address: selected, name: "Mime Device", last_seen: -1};
    }else{
